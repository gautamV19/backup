...
const TaskForm: FC = () => {
  ...
  const handleCategoriesSelection = (catId: string) => {
    ...
    if (categoryIds.find((id) => id === catId) === undefined) {
      ...
      setCategoryIds((selectedCategories) =>
        selectedCategories.filter((cat) => cat !== catId)
      );
    }
  };
  ...
  const handleTaskSubmit = async () => {
    if (user) {
      try {
        let taskOrder = taskId
          ? location.state.order
          : tasks.filter(
              (task) => task.project === projectId && task.phase === phase
            ).length + 1;
        ...
        if (location.state.phase !== phase) {
          ...
          const tasksToUpdate = tasks.filter(
            (task) =>
              ...
              task.phase === location.state.phase &&
              task.order > taskOrderOriginal
          );
        ...
        const taskDoc: TaskReference = taskId
      ...
      <CardActions>
        ...
        {taskId && (
          <DialogOpennerWrapper
            message={
              <Typography>
                This action will permanently delete task: <b>{name}</b>
              </Typography>
            }
