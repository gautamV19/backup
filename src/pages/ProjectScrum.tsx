...
const ProjectScrum: FC = () => {
  ...
  const handleDeleteCategory = (categoryId: string | undefined) => {
    ...
    for (const task of tasks) {
      const updatedCategory = task.category.filter(
  ...
  const handleCheckboxToggle = (category: Category) => () => {
    ...
    if (
      Object.entries(newChecked).filter(
        (check: [string, number]) => check[1] === 1
      ).length === 0
    ) {
  ...
  const onDragEnd = (result: any) => {
    ...
    if (destination.droppableId === source.droppableId) {
      const sourceToChange = tasks.filter(
        (task) =>
          task.project === projectId &&
          task.phase === source.droppableId &&
          task.order === source.index + 1
      )[0];
      const destinationToChange = tasks.filter(
        (task) =>
          task.project === projectId &&
          task.phase === destination.droppableId &&
      ...
      try {
        ...
        if (source.index > destination.index) {
          ...
          const sourceTasksToUpdate = tasks.filter(
            (task) =>
              task.project === projectId &&
              task.phase === source.droppableId &&
              task.order >= destinationToChange.order &&
              task.order < sourceToChange.order
          );
          ...
          tasksCollection
            .doc(sourceToChange.id)
            .update({ order: destinationToChange.order });
       ...
       * Upravim destination
       */
      const destinationTasksToUpdate = tasks.filter(
        (task) =>
          task.project === projectId &&
          task.phase === destination.droppableId &&
      ...
      if (taskToChange) {
        ...
        try {
          ...
          tasksCollection.doc(taskToChange.id).update({
          ...
          <List className={classes.listRoot}>
            <ListSubheader>
              <Grid container item spacing={2} direction="column">
                ...
                <Grid item>
                  {user && project && user.uid === project.by.uid && (
                    <Link
                      className={classes.link}
                      to={{
                ...
                >
                  ...
                  <ListItemSecondaryAction>
                    ...
                    <DialogOpennerWrapper
                      message={
                        <Typography>
                          This action will permanently delete category:{" "}
                          <b>{category.name}</b>
                        </Typography>
                      }
                      deleteCallback={() => handleDeleteCategory(category.id)}
                      openComponent={(openCallback) => (
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <Grid container item xs={12} sm={12} md={9} spacing={1}>
            <Grid item xs={12} sm={6} md={3}>
              <BoardColumn
                title={"TO DO"}
                tasks={filterTasksByPhase(filteredTasks, "TO DO")}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <BoardColumn
                title={"IN PROGRESS"}
                tasks={filterTasksByPhase(filteredTasks, "IN PROGRESS")}
                categories={categories}
              />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <BoardColumn
                title={"TESTING"}
                tasks={filterTasksByPhase(filteredTasks, "TESTING")}
                categories={categories}
              />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <BoardColumn
                title={"DONE"}
                tasks={filterTasksByPhase(filteredTasks, "DONE")}
                categories={categories}
              />
            </Grid>
          </Grid>
